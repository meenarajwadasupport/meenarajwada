...
export default function HeroSlider() {
  ...
  // One ref per slide video, keyed by slide id
  ...
  return (
    <section className={`relative overflow-hidden -mt-[76px] ${sliderHeight}`}>
      ...
      {slides.map((slide, idx) => {
        ...
        const gif = hasVideo && isGif(slide.video_url)
        const mp4 = hasVideo && !gif
      ...
      {/* ── EMBLA — text/CTA overlay only, no backgrounds inside ── */}
      <div ref={emblaRef} className={`absolute inset-0 overflow-hidden`}>
        <div className="flex h-full">
          {slides.map((slide, idx) => (
            <div key={slide.id} className="relative flex-[0_0_100%] h-full flex items-center justify-center text-center px-5 sm:px-8">
      ...
      {slides.length > 1 && (
        <>
          <button
            ...
            className="hidden sm:flex absolute left-5 lg:left-8 top-1/2 -translate-y-1/2 w-11 h-11 items-center justify-center bg-white/[0.08] hover:bg-white/25 backdrop-blur-md rounded-full text-white transition-all duration-300 border border-white/25 hover:scale-110 active:scale-95 z-10"
          >
          ...
          <button
            ...
            className="hidden sm:flex absolute right-5 lg:right-8 top-1/2 -translate-y-1/2 w-11 h-11 items-center justify-center bg-white/[0.08] hover:bg-white/25 backdrop-blur-md rounded-full text-white transition-all duration-300 border border-white/25 hover:scale-110 active:scale-95 z-10"
          >
            <ChevronRight className="w-5 h-5" />
          </button>

          {/* Dot indicators */}
          <div className="absolute bottom-14 left-1/2 -translate-x-1/2 flex items-center gap-2.5 z-10">
            {slides.map((_, i) => (
              <button
                key={i}
      ...
      {/* Bottom ticker */}
      <div className="absolute bottom-0 left-0 right-0 overflow-hidden flex items-center border-t border-white/10 z-10" style={{ height: '30px', background: 'linear-gradient(90deg, #7D1935 0%, #9e2044 50%, #7D1935 100%)' }}>
