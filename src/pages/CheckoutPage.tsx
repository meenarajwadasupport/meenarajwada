  ...
  async function onSubmit(data: FormData) {
    ...
    try {
      ...
      const res = await fetch('/api/create-cashfree-order', {
        ...
        body: JSON.stringify({ order_id: order.id, amount: total, customer: { name: data.full_name, email: data.email, phone: data.phone } }),
      })
      const { payment_session_id } = await res.json()
      if (!payment_session_id) throw new Error('Payment session failed')

      clearCart()
      await startCashfreePayment(payment_session_id, `${window.location.origin}/payment-status?order_id=${order.id}`)
    } catch (err: any) {
          ...
          <div className="space-y-4">
            <div className="bg-white rounded-2xl p-5 border border-border">
              ...
              <div className="space-y-3 max-h-52 overflow-y-auto">
                {items.map(item => (
                  <div key={`${item.product_id}-${item.size}`} className="flex gap-3">
                    <img src={item.product?.images?.[0]} alt="" className="w-12 h-12 object-cover rounded-lg bg-muted flex-shrink-0" />
              ...
              <div className="border-t border-border mt-4 pt-4 space-y-2 text-sm">
                ...
                <div className="flex justify-between text-muted-foreground"><span>Shipping</span><span className={shipping === 0 ? 'text-green-600 font-medium' : ''}>{shipping === 0 ? 'FREE' : formatPrice(shipping)}</span></div>
